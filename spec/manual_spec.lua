-- The Manual's pure core (ADR-0008): IR -> backend payloads. The GUI rendering and
-- the live remote registration are only exercisable in the full client; here we pin
-- the engine-free shape both adapters are built from.
...
describe("Booktorio adapter", function()
  it("maps every chapter to a topic, header row first in tables", function()
    local thread = ir.booktorio_thread(content)
    assert.are.equal("lamented-cpu", thread.mod)
    assert.are.equal(#content, #thread.topics)
    assert.are.equal(content[1].title, thread.topics[1].name)

    -- the Overview's extension table -> a Booktorio TopicTable with the header row first
    local table_el
    for _, el in ipairs(thread.topics[1].topic) do
      if el.type == "table" then
        table_el = el
      end
    end
    assert.is_truthy(table_el)
    assert.are.same({ "Extension", "Accepted" }, table_el.table[1])
  end)
end)
